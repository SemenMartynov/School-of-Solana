...
impl Calculator {
    ...
    pub fn addition(&mut self, x: i64, y: i64) -> Option<i64> {
        let opt = OperationType::Addition;

        let res = opt.perform(x, y);

        if let Some(_) = res {
            self.history.push(Operation::new(x, y, opt));
        }
    ...
    pub fn subtraction(&mut self, x: i64, y: i64) -> Option<i64> {
        let opt = OperationType::Subtraction;
    ...
    pub fn multiplication(&mut self, x: i64, y: i64) -> Option<i64> {
        let opt = OperationType::Multiplication;
    ...
    pub fn show_history(&self) -> String{
        ...
        for (pos, val) in self.history.iter().enumerate() {
            let res = val.operation_type.perform(val.first_num, val.second_num);
            let res = res.unwrap_or(0);
            result.push_str(&format!("{}: {} {} {} = {}\n", pos, val.first_num, val.operation_type.get_sign(), val.second_num, res));
        };
        result
    }
    ...
    pub fn repeat(&mut self, operation_index: usize) -> Option<i64>{
        ...
        let opt = op.operation_type.clone();
        let x = op.first_num;
        let y = op.second_num;
